...
def get_pyqt_configuration(options):
    ...
    """
    if options.qt == 5: return get_pyqt5_configuration()
    else : return get_pyqt4_configuration()
...
def get_pyqt5_configuration():
    """Return the PyQt configuration for Qt5.
    """
    import PyQt5.QtCore 
    import sipconfig
    from os.path import join
    import sys
    pyqtconfig =  PyQt5.QtCore.PYQT_CONFIGURATION
    pyqtconfig = dict([('pyqt_'+key, value) for key,value in pyqtconfig.items()])
    pyqtconfig['pyqt_sip_dir'] = join(sys.prefix,'share','sip','PyQt5')
    pyqtconfig['qt_version'] = PyQt5.QtCore.QT_VERSION
    pyqtconfig['qt_threaded'] = 1
    pyqtconfig['qt_winconfig'] = 'shared'
    pyqtconfig['qt_framework'] = True

    config = sipconfig.Configuration([pyqtconfig])
    return config
...
def setup_qglviewer_config( options, package ):
    from os.path import join
    qt_version = options.qt
    configfile = file(join('src','sip','QGLViewerModuleConfig.sip'),'w')
    configfile.write('// Config file automatically generated by configure.py\n\n')
    configfile.write('%Module(name='+package+'Qt'+str(qt_version)+')\n\n')
...
def setup_qglviewer_build(configuration, options, package):
    ...
    sip_install_dir = os.path.abspath( os.path.join( pyqt_sip_dir, os.path.pardir, 'QGLViewer'))
    if options.verbose_config:
...
def parse_args():
    ...
    common_options.add_option(
        ...
        help=('force the import of glteximage3d extention'
              ' [default disabled]'))
    common_options.add_option(
        '--qt', default=4, action='store',type='int',
        help=('Select the qt version to use [default=4]'))
