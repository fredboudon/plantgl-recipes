...
def execProgram(program, *args):
    ...
    #child.communicate()
    return child.returncode, str(child.stdout)
...
class Repository:
    ...
    def update(self):
        ...
        if not os.path.exists(self.name) :
            ...
            if self.localRepoOwner() != self.owner :
                ...
                os.chdir(self.name)
                execGit('checkout','-b',self.branch)
                execGit('pull')
                os.chdir(os.pardir)
            else:
                os.chdir(self.name)
    ...
    def removePreviousSubmodule(self):
        if os.path.exists(self.name):
            execGit('rm', '-f', self.name)

    def localRepoOwner(self):
        remotesignature = 'origin   https://github.com/'
        if os.path.exists(self.name):
            os.chdir(self.name)
            code, value = execGit('remote','-v')
            os.chdir(os.pardir)
            if code == 0 and value.startswith(remotesignature):
